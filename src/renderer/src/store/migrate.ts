...
const migrateConfig = {
  ...
  '207': (state: RootState) => {
    try {
      // Rename 黑洞AI -> blackhole AI in persisted assistant data
      if (state.assistants?.defaultAssistant?.name?.includes('黑洞AI')) {
        state.assistants.defaultAssistant.name = state.assistants.defaultAssistant.name.replace('黑洞AI', 'blackhole AI')
      }
      if (state.assistants?.assistants) {
        state.assistants.assistants.forEach((assistant) => {
          if (assistant.name?.includes('黑洞AI')) {
            assistant.name = assistant.name.replace('黑洞AI', 'blackhole AI')
          }
        })
      }
      // Add monitor to sidebar icons if not present
      if (state.settings?.sidebarIcons?.visible && !state.settings.sidebarIcons.visible.includes('monitor')) {
        const translateIndex = state.settings.sidebarIcons.visible.indexOf('translate')
        if (translateIndex !== -1) {
          state.settings.sidebarIcons.visible = [
            ...state.settings.sidebarIcons.visible.slice(0, translateIndex + 1),
            'monitor',
            ...state.settings.sidebarIcons.visible.slice(translateIndex + 1)
          ]
        } else {
          state.settings.sidebarIcons.visible = [...state.settings.sidebarIcons.visible, 'monitor']
        }
      }
