...
interface Props {
  ...
  topic: Topic
  /** 外部拖拽注入的文件列表（用于 DragUploadProvider 集成） */
  droppedFiles?: FileMetadata[]
}
...
const Inputbar: FC<Props> = ({ assistant: initialAssistant, setActiveTopic, topic, droppedFiles }) => {
...
const InputbarInner: FC<InputbarInnerProps> = ({
  ...
  useEffect(() => {
    if (droppedFiles && droppedFiles.length > 0) {
      ...
      const newFiles = droppedFiles.filter((f) => !existingIds.has(f.id))
