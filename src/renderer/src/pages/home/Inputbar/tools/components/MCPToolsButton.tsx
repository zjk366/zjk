...
import { setGlobalAutoMode } from '@renderer/pages/home/Messages/Tools/hooks/useMcpToolApproval'
import { CircleX, Download, Hammer, Plus, Sparkles } from 'lucide-react'
...
const MCPToolsButton: FC<Props> = ({ quickPanel, setInputValue, resizeTextArea, assistantId }) => {
  ...
  const isActive = currentMode === 'auto'
  ...
  return (
    <>
      <Tooltip placement="top" title={t('settings.mcp.title')} mouseLeaveDelay={0} arrow>
        <ActionIconButton onClick={handleToggleMode} onDoubleClick={handleInstallPackage} active={isActive} aria-label={t('settings.mcp.title')}>
          {getButtonIcon()}
        </ActionIconButton>
      </Tooltip>
      <Modal
        ...
        onOk={() => {
          if (installPkg.trim()) {
            import('@renderer/store/mcp').then((mod) => {
              mod.installMcpPackage(installPkg.trim())
              window.toast.success(`正在安装 MCP: ${installPkg.trim()}`)
            })
          }
        ...
        cancelText="取消">
        <Input
          placeholder="输入 npm 包名，如 @playwright/mcp"
          value={installPkg}
