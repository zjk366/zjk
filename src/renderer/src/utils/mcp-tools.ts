...
/** 全局 MCP 自动模式标志（与 useMcpToolApproval 共享同一 key）*/
const GLOBAL_AUTO_MODE_KEY = '__mcp_auto_mode'

function getGlobalAutoMode(): boolean {
  const mem = !!(window as any)[GLOBAL_AUTO_MODE_KEY]
  if (mem) return true
  const stored = localStorage.getItem('mcp_auto_mode')
  if (stored === 'true') {
    (window as any)[GLOBAL_AUTO_MODE_KEY] = true
    return true
  }
  return false
}
...
export async function callMCPTool(
  ...
  try {
    ...
    // 保存图片到文件库（如已配置）
    if (resp?.content) {
      const images = resp.content.filter((c: any) => c.type === 'image' && c.data)
      if (images.length > 0) {
        setTimeout(async () => {
          try {
...
export function isToolAutoApproved(tool: MCPTool, server?: MCPServer, allowedTools?: string[]): boolean {
  // 全局自动模式：所有工具自动放行（由 ✨ 按钮切换，持久化到 localStorage）
  if (getGlobalAutoMode()) {
    return true
  }
  if (tool.isBuiltIn) {
    return true
