...
/**
 * Creates a Compressed Message Block for smart context compression.
 * @param messageId - The ID of the parent message.
 * @param summary - The compression summary text generated by LLM.
 * @param originalMessageCount - Number of original messages compressed.
 * @param originalTokenCount - Original token count before compression.
 * @param compressedTokenCount - Token count after compression.
 * @param overrides - Optional properties to override the defaults.
 * @returns A CompressedMessageBlock object.
 */
export function createCompressedBlock(
  messageId: string,
  summary: string,
  ...
  overrides: Partial<
    Omit<
      CompressedMessageBlock,
      'id' | 'messageId' | 'type' | 'summary' | 'originalMessageCount' | 'originalTokenCount' | 'compressedTokenCount'
    >
  > = {}
): CompressedMessageBlock {
  const baseBlock = createBaseMessageBlock(messageId, MessageBlockType.COMPRESSED, overrides)
  return {
    ...baseBlock,
    summary,
