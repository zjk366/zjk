...
export function useAppInit() {
  ...
  useEffect(() => {
    ...
    const requestListener = async (_event: Electron.IpcRendererEvent, payload: ToolPermissionRequestPayload) => {
      ...
      // MCP auto mode → skip permission dialog
      if (!payload.autoApprove) {
        try {
          const asst = store.getState().assistants?.defaultAssistant
          if (asst && getEffectiveMcpMode(asst) === 'auto') {
            const r = await window.api.agentTools.respondToPermission({
              requestId: payload.requestId, behavior: 'allow',
              updatedInput: payload.input, updatedPermissions: payload.suggestions
            })
            if (r?.success) { logger.debug(`Auto-approved: ${payload.toolName}`); return }
          }
        } catch (_) { /* skip */ }
      }
  ...
  // 初始化内置 MCP 服务器（延迟 3 秒等 Redux 就绪）
  useEffect(() => {
    // 立即恢复 MCP 自动模式标志（从 Redux 读取）
    try {
      const asst = store.getState().assistants?.defaultAssistant
      if (asst && asst.mcpMode === 'auto' && !(window as any).__mcp_auto_mode) {
        (window as any).__mcp_auto_mode = true
      }
    } catch { /* ok */ }

    // 监听 Hub 图片保存事件 → 写入文件库
    const onUpdated = (window as any).api?.onSkillsUpdated
    const unsub = typeof onUpdated === 'function'
      ? onUpdated((data: any) => {
          if (data?.command === 'hub-image-saved' && data?.images?.length > 0) {
            const libPath = localStorage.getItem('filelib_path')
            if (!libPath) return
            data.images.forEach(async (srcPath: string) => {
              try {
                ...
                // 读取临时文件内容并写入文件库目录
                const raw = await (window as any).api?.fs?.read(srcPath)
                if (raw) {
                  await (window as any).api?.file?.write(`${dir}/${name}`, raw instanceof Uint8Array ? raw : new Uint8Array(raw))
                }
              } catch { /* skip */ }
            })
            window.toast?.success?.(`截图已保存到文件库 (${data.images.length} 张)`)
          }
        })
      : undefined
    ...
    const timer = setTimeout(() => {
      ...
      import('@renderer/store/mcp').then(({ initializeMCPServers }) => {
        initializeMCPServers(servers, store.dispatch)
      })
    }, 3000)
