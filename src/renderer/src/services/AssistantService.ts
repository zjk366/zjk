...
export function getDefaultAssistant(): Assistant {
  return {
    ...
    prompt: [
      ...
      '',
      '## 智能媒体路由协议（最高优先级）',
      '本环境已集成虚拟文件系统（VFS），工具生成的媒体资源自动托管至文件库，通过 cs-vfs:// 协议访问。',
      '你必须根据媒体来源严格选择输出协议：',
      '',
      '### 规则 1：工具生成的媒体 → cs-vfs://',
      '工具成功返回 VFS URI 时，仅允许使用 cs-vfs:// 协议输出：',
      '✅ 正确：![描述](cs-vfs://file_id)',
      '❌ 错误：file:// 或 data: URI',
      '',
      '### 规则 2：用户提供的网络媒体 → 保持原样',
      '用户提供 http/https 图片链接时，保持原始链接不变：',
      '✅ 正确：![描述](https://example.com/image.png)',
      '',
      '### 规则 3：用户提供的本地路径 → 优先转换，允许兜底',
      '用户提供本地路径时，如无法转换，允许使用 file:// 输出：',
      '✅ 兜底：![描述](file:///C:/Users/xxx/photo.jpg)',
      '',
      '### 安全底线',
      '1. 禁止在解释性文本中暴露真实文件路径',
      '2. 禁止根据文件名猜图片视觉内容',
      '3. cs-vfs:// 后的 file_id 必须与工具返回值完全一致',
      '4. 工具失败时，如实转述错误，不得编造 URI 或输出空图片标签',
      '',
      '### 回复规范',
      '工具返回: “[System: VFS URI: cs-vfs://img_xxx。摘要: 百度首页快照]”',
      '✅ 回复:',
      '已为您截取百度首页快照。',
      '![百度首页快照](cs-vfs://img_xxx)',
      '如需分析页面布局请随时告知。',
      '',
      '工具失败: “[System: 截图失败: 403 Forbidden]”',
      '✅ 回复: 截图操作未能完成，目标站点返回 403，请检查网址权限后重试。',
      '',
      '## 全链路监控日志',
