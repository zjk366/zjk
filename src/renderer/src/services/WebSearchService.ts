...
class WebSearchService {
  ...
  public async processWebsearch(
    ...
    // 自动兜底：如果搜索结果为空，依次尝试其他本地搜索提供商
    const FALLBACK_CHAIN: WebSearchProviderId[] = ['local-bing', 'local-google', 'local-baidu']
    if (finalResults.length === 0 && FALLBACK_CHAIN.includes(webSearchProvider.id)) {
      const currentIdx = FALLBACK_CHAIN.indexOf(webSearchProvider.id)
      for (let fi = currentIdx + 1; fi < FALLBACK_CHAIN.length; fi++) {
        const fallbackId = FALLBACK_CHAIN[fi]
        try {
          logger.info(`[Fallback] ${webSearchProvider.id} returned 0 results, trying ${fallbackId}...`)
          const fallbackProvider: WebSearchProvider = {
            ...webSearchProvider,
            id: fallbackId,
            name: fallbackId,
          }
          const fallbackPromises = questions.map((q) =>
            this.search(fallbackProvider, q, { signal }, span?.spanContext().spanId)
          )
          const fallbackResults = await Promise.allSettled(fallbackPromises)
          const fallbackItems: WebSearchProviderResult[] = []
          fallbackResults.forEach((r) => {
            if (r.status === 'fulfilled' && r.value.results) {
              fallbackItems.push(...r.value.results)
            }
          })
          if (fallbackItems.length > 0) {
            logger.info(`[Fallback] ${fallbackId} returned ${fallbackItems.length} results`)
            finalResults = fallbackItems
            break
          }
          logger.info(`[Fallback] ${fallbackId} also returned 0 results`)
        } catch (fallbackErr) {
          logger.warn(`[Fallback] ${fallbackId} failed:`, fallbackErr)
        }
      }
    }
