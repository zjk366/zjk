...
const getTabIcon = (
  ...
  switch (tabId) {
    case 'blackhole':
      return <img src="/src/assets/images/blackhole.png" style={{ width: 14, height: 14, borderRadius: '50%' }} alt="Blackhole" />
    case 'home':
      return <img src="/src/assets/images/blackhole.png" style={{ width: 14, height: 14, borderRadius: '50%' }} alt="文字黑洞工作区" />
    case 'monitor':
...
const specialTabs = ['launchpad', 'settings', 'agents', 'monitor', 'skills', 'memory']
...
const TabsContainer: React.FC<TabsContainerProps> = ({ children }) => {
  ...
  const { useSystemTitleBar, narrowMode } = useSettings()
  ...
  const shouldCreateTab = (path: string) => {
    ...
    if (path === '/filelib') return false
    if (path === '/files') return false
