...
class FileStorage {
  ...
  /** 递归扫描目录，列出所有文件及其元数据（无大小限制） */
  public listAllFiles = async (_: Electron.IpcMainInvokeEvent, dirPath: string): Promise<FileMetadata[]> => {
    const MAX_FILES = 5000
    const MAX_DEPTH = 20
    const EXCLUDED_DIRS = new Set([
      'node_modules', '.git', '.idea', '.vscode', 'dist', 'build', '.next',
      '.nuxt', 'coverage', '.cache', '__pycache__', 'vendor', '.svn',
      '.DS_Store', 'Thumbs.db', 'bin', 'obj', '.gradle', 'target'
    ])
    const EXCLUDED_PREFIXES = ['/System/', '/proc/', '/sys/', '/dev/', '/run/', '/snap/', '/Windows/', 'C:\\Windows\\']

    const safeDirPath = path.resolve(dirPath)

    // Security: reject system paths
    if (EXCLUDED_PREFIXES.some((prefix) => safeDirPath.startsWith(prefix) || safeDirPath === prefix.slice(0, -1))) {
      throw new Error('System directories are not allowed')
    }
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            results.push({
              ...
              size: stats.size,
              ext: ext,
              type: (await this.getFileType(fullPath)) as FileType,
              count: 1
            })
          } catch {
