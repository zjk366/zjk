...
class CherryINOAuthService {
  ...
  public startOAuthFlow = async (
    ...
    // Build authorization URL (Auth0 endpoint)
    const authUrl = new URL(`${oauthServer}/authorize`)
  ...
  public exchangeToken = async (
    ...
    try {
      ...
      const tokenResponse = await net.fetch(`${oauthServer}/oauth/token`, {
        method: 'POST',
        headers: {
      ...
      // Auth0 mode: use access token directly as the API key
      // (no separate API key endpoint like CherryIN's /api/v1/oauth/tokens)
      if (!accessToken) {
        throw new CherryINOAuthServiceError('No access token received')
      }

      logger.debug('Using access token as API key')
      return { apiKeys: accessToken }
    } catch (error) {
  ...
  private refreshAccessToken = async (apiHost: string): Promise<string | null> => {
    try {
      ...
      const response = await net.fetch(`${apiHost}/oauth/token`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        },
  ...
  /**
   * Get user balance (Auth0 mode: returns 0 as Auth0 has no balance API)
   */
  public getBalance = async (_: Electron.IpcMainInvokeEvent, apiHost: string): Promise<BalanceResponse> => {
    ...
    try {
      ...
      // Auth0 has no balance API; return 0 gracefully
      logger.warn('Balance API unavailable (expected with Auth0):', (error as Error).message)
      return { balance: 0 }
    }
  }
  ...
  public logout = async (_: Electron.IpcMainInvokeEvent, apiHost: string): Promise<void> => {
    ...
    try {
      ...
      if (token) {
        try {
          await net.fetch(`${apiHost}/oauth/revoke`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded'
            },
