...
export async function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
  ...
  ipcMain.handle(IpcChannel.File_ListAllFiles, fileManager.listAllFiles.bind(fileManager))
  ...
  // 清除 MCP 缓存，确保新启用的服务立即生效
  ipcMain.handle(IpcChannel.Mcp_ClearCache, () => {
    const { CacheService } = require('@main/services/CacheService')
    CacheService.remove('api-server:mcp-servers')
  })
  ...
  // 保存文件库路径到配置文件
  ipcMain.handle('app:set-filelib-path', (_, libPath: string) => {
  ...
  ipcMain.handle(IpcChannel.App_OpenTerminal, async (_e, command: string, packageName?: string) => {
    const { spawn } = require('node:child_process')
    const isWin = process.platform === 'win32'
    try {
      if (isWin) {
        const child = spawn('cmd.exe', ['/k', command], { windowsHide: false, detached: false })
        child.on('exit', async () => {
          // 通知渲染进程更新
          const mainWin = windowService.getMainWindow()
          if (mainWin && !mainWin.isDestroyed()) {
            mainWin.webContents.send(IpcChannel.Skill_Updated, { command, packageName, finished: true })
          }
          // 尝试热加载：重启对应 MCP 服务器
          if (packageName) {
            try {
        ...
        child.on('error', (e: any) => {
          const mainWin = windowService.getMainWindow()
          if (mainWin && !mainWin.isDestroyed()) {
            mainWin.webContents.send(IpcChannel.Skill_Updated, { command, error: e.message })
          }
        })
        ...
        spawn('x-terminal-emulator', ['-e', `bash -c "${command}; exec bash"`], { detached: true })
      }
    } catch (err: any) {
      const mainWin = windowService.getMainWindow()
      if (mainWin && !mainWin.isDestroyed()) {
        mainWin.webContents.send(IpcChannel.Skill_Updated, { command, error: err.message })
      }
    }
  })
  ...
  // Portal callback server (blackhole login)
  ipcMain.handle(IpcChannel.Portal_StartServer, async () => {
      ...
      const server = http.createServer((req, res) => {
        ...
        try {
          ...
          if (url.pathname === '/config') {
            ...
            if (mainWindow && !mainWindow.isDestroyed()) {
              mainWindow.webContents.send(IpcChannel.Portal_ApiKey, {
                apiKey,
                baseUrl
              })
            }
            ...
            // Close server shortly after request is handled
            setTimeout(() => {
              try {
                server.close()
              } catch {
